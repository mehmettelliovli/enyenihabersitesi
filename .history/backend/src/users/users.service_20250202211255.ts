...
export class UsersService {
  ...
  async delete(id: number): Promise<void> {
    ...
    // Kullanıcının haberlerini de pasif yap
    await this.userRepository.manager.transaction(async (manager) => {
      // Kullanıcıyı pasif yap
      await manager.update(User, id, { isActive: false });
      
      // Kullanıcının haberlerini pasif yap
      await manager.update(News, 
        { author: { id: user.id } },
        { isActive: false }
      );
      
      // Kullanıcının rol mappinglerini pasif yap
      await manager.update(UserRoleMapping,
        { user: { id: user.id } },
        { isActive: false }
      );
    });
